...
def create_workspace(workspace_name, capacity_id, token):
    ...
    try:
        ...
        if response.status_code != 201:
            # If workspace is not created then raise exception
            json_response_str = None
            try:
                json_response_str = str(response.json())
            except Exception as e:
                json_response_str = str(response)
            raise Exception(f"Unable to create workspace: {json_response_str}")
        
        # If workspace is created return workspace id    
